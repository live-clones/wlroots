...
static void render_pass_add_blur(struct wlr_render_pass *wlr_pass,
	...
	int bx = options->box.x, by = options->box.y;
	int bw = options->box.width, bh = options->box.height;
	if (bw <= 0 || bh <= 0) {
		return;
	}

	int buf_w = render_buffer->wlr_buffer->width;
	int buf_h = render_buffer->wlr_buffer->height;
	if (bx < 0) {
		bw += bx; bx = 0;
	}
	if (by < 0) {
	...
	if (!vulkan_blur_ensure_scratch(renderer, bw, bh)) {
		wlr_log(WLR_ERROR, "vulkan blur: failed to ensure scratch images");
		pass->failed = true;
		return;
	}
	...
	for (int i = 0; i < 2; i++) {
		if (!renderer->blur.transitioned[i]) {
			renderer->blur.transitioned[i] = true;
...

	{
		...
		vkCmdCopyImage(cb, render_buffer->two_pass.blend_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
			renderer->blur.image[0], VK_IMAGE_LAYOUT_GENERAL, 1, &copy);
	}
	...
	wlr_matrix_scale(m, bw, bh);
	wlr_matrix_multiply(m, scratch_proj, m);

	struct wlr_vk_vert_pcr_data vert_pcr = {
		.uv_off = { 0.f, 0.f },
		.uv_size = { 1.f, 1.f },
	};
	encode_proj_matrix(m, vert_pcr.mat4);

	VkViewport scratch_vp = {
		.width = bw,
		.height = bh,
		.maxDepth = 1.f
	};
...

	{
		VkRenderPassBeginInfo rp_info = {
			...
			.framebuffer = renderer->blur.fb[1],
			.renderArea = {
				.extent = {
					.width = bw,
					.height = bh
		...
		vkCmdSetViewport(cb, 0, 1, &scratch_vp);
		vkCmdSetScissor(cb, 0, 1, &full_scissor);
		vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->blur.pipeline);
		vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->blur.pipe_layout, 0, 1,
			&renderer->blur.ds[0], 0, NULL);
		vkCmdPushConstants(cb, renderer->blur.pipe_layout, VK_SHADER_STAGE_VERTEX_BIT, 0,
			sizeof(vert_pcr), &vert_pcr);
		float h_step[2] = { 1.f / bw, 0.f };
		vkCmdPushConstants(cb, renderer->blur.pipe_layout, VK_SHADER_STAGE_FRAGMENT_BIT,
			sizeof(vert_pcr), sizeof(h_step), h_step);
		vkCmdDraw(cb, 4, 1, 0, 0);
		vkCmdEndRenderPass(cb);
	}
...

	{
		VkRenderPassBeginInfo rp_info = {
			.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,
			.renderPass = renderer->blur.render_pass,
			.framebuffer = renderer->blur.fb[0],
			.renderArea = {
				.extent = {
					.width = bw,
					.height = bh
		...
		vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->blur.pipeline);
		vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer->blur.pipe_layout, 0, 1,
			&renderer->blur.ds[1], 0, NULL);
		vkCmdPushConstants(cb, renderer->blur.pipe_layout, VK_SHADER_STAGE_VERTEX_BIT, 0,
			sizeof(vert_pcr), &vert_pcr);
		float v_step[2] = { 0.f, 1.f / bh };
		vkCmdPushConstants(cb, renderer->blur.pipe_layout, VK_SHADER_STAGE_FRAGMENT_BIT,
			sizeof(vert_pcr), sizeof(v_step), v_step);
		vkCmdDraw(cb, 4, 1, 0, 0);
		vkCmdEndRenderPass(cb);
	}
...

	{
		...
		pixman_region32_t full_box_region;

		if (options->clip != NULL) {
			clip_rects = pixman_region32_rectangles(options->clip, &clip_rects_len);
		} else {
			pixman_region32_init_rect(&full_box_region, bx, by, bw, bh);
			clip_rects = pixman_region32_rectangles(&full_box_region, &clip_rects_len);
		}

		for (int i = 0; i < clip_rects_len; i++) {
			const pixman_box32_t *r = &clip_rects[i];
			VkImageCopy copy = {
				.srcSubresource = {
					.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
					.mipLevel = 0,
			...
			vkCmdCopyImage(cb, renderer->blur.image[0], VK_IMAGE_LAYOUT_GENERAL,
				render_buffer->two_pass.blend_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy);
		}

		if (options->clip == NULL) {
			pixman_region32_fini(&full_box_region);
		}
	}
	...
	{
		int width = render_buffer->wlr_buffer->width;
		int height = render_buffer->wlr_buffer->height;
	...
	render_pass_mark_box_updated(pass, &options->box);
}
