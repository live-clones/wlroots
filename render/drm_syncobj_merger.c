...
void wlr_drm_syncobj_merger_commit_and_destroy(struct wlr_drm_syncobj_merger *merger) {
...
static void export_waiter_handle_ready(struct wlr_drm_syncobj_timeline_waiter *waiter) {
	...
	wlr_drm_syncobj_merger_commit_and_destroy(add->merger);
...
bool wlr_drm_syncobj_merger_add(struct wlr_drm_syncobj_merger *merger,
	...
	if (exportable) {
		...
		wlr_log(WLR_ERROR, "point is not materialized, but no event_loop was provided");
		return false;
	}
