...
static void merger_add_callback(struct wlr_drm_syncobj_timeline_waiter* waiter) {
...
bool wlr_drm_syncobj_merger_add(struct wlr_drm_syncobj_merger *merger,
	...
	if (exportable) {
		...
		merger->n_ref++;
		return wlr_drm_syncobj_timeline_waiter_init(&add->waiter, src_timeline, src_point,
			flags, loop, merger_add_callback);
	} else {
		return false;
	}
}
