...
static bool read_surface_input_shape(struct wlr_xwm *xwm,
		struct wlr_xwayland_surface *xsurface) {
	...
	bool bounding_ok = read_surface_shape_region(xwm, xsurface, bounding_cookie,
		&bounding_region);
	bool input_ok = read_surface_shape_region(xwm, xsurface, input_cookie,
		&input_region);
...
static void update_surface_input_shape(struct wlr_xwm *xwm,
		struct wlr_xwayland_surface *xsurface) {
	xsurface->input_shape_dirty = false;
	if (!read_surface_input_shape(xwm, xsurface)) {
		xsurface->input_shape_dirty = true;
	}
}
...
static struct wlr_xwayland_surface *xwayland_surface_create(
	...
	if (xwm->shape_input_supported) {
		...
		update_surface_input_shape(xwm, surface);
	}
...
static void xwm_handle_configure_notify(struct wlr_xwm *xwm,
	...
	bool size_changed = xsurface->width != ev->width ||
	...
	if (xwm->shape_input_supported &&
			(size_changed || xsurface->input_shape_dirty)) {
		update_surface_input_shape(xwm, xsurface);
	}
