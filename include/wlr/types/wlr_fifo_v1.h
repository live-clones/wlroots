...
struct wlr_fifo_manager_v1_fifo_barrier_event {
...
struct wlr_fifo_manager_v1 {
	...
	struct {
		/**
		 * Signals that the fifo barrier has been set on the surface.
		 * The listener is responsible for clearing this barrier.
		 */
		struct wl_signal barrier_set;
...
struct wlr_fifo_v1 {
	...
	/* the surface commit sequence number for which the fifo barrier applies */
	uint64_t barrier_commit_seq;

	/* the fifo barrier for the surface */
	bool fifo_barrier;
...
/**
 * Used to clear the fifo barrier for the surface.
 */
