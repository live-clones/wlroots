...
struct wlr_commit_timer_v1 {
	...
	/**
	 * When the surface has not yet been mapped and we receive a .get_timer request, there is still
	 * no corresponding scene_surface in the scene, so we need to delay the setup until after the
	 * surface is mapped.
	 */
	struct wl_listener map; // wlr_surface.map
