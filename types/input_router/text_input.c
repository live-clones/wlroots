...
// XXX: improve wlr_text_input_v3 focus API
...
static void update_active_text_input(struct wlr_text_input_v3_input_router_layer *layer) {
	...
	struct wlr_surface *surface = wlr_input_router_focus_get_surface(&layer->focus);
	if (surface != NULL) {
...
static void create_text_input(struct wlr_text_input_v3_input_router_layer *layer,
	...
	text_input_safe_enter(text_input, wlr_input_router_focus_get_surface(&layer->focus));
	update_active_text_input(layer);
}
...
static void keyboard_handler_set_focus(struct wlr_input_router_keyboard_handler *handler,
	...
	wlr_input_router_focus_copy(&layer->focus, event->focus);

	// Relay the event first; "the text-input focus follows the keyboard focus".
	wlr_input_router_keyboard_handler_relay_set_focus(handler, event);
...
static const struct wlr_input_router_keyboard_handler_interface keyboard_handler_impl = {
	.base = {
		.name = "wlr_text_input_v3_input_router_layer",
	},
...
struct wlr_text_input_v3_input_router_layer *wlr_text_input_v3_input_router_layer_create(
	...
	wlr_input_router_invalidate_keyboard_focus(router);
