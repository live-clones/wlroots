...
static void color_repr_handle_set_alpha_mode(struct wl_client *client,
	...
	if (color_repr == NULL) {
		wl_resource_post_error(resource, WP_COLOR_REPRESENTATION_V1_ERROR_INERT,
			"Associated surface has been destroyed, object is inert");
		return;
	}

	if (alpha_mode != WP_COLOR_REPRESENTATION_V1_ALPHA_MODE_PREMULTIPLIED_ELECTRICAL) {
		wl_resource_post_error(resource,
			WP_COLOR_REPRESENTATION_V1_ERROR_UNSUPPORTED_ALPHA_MODE,
...
static void color_repr_handle_set_coefficients_and_range(struct wl_client *client,
	...
	enum wlr_color_repr_encoding wlr_encoding =
		wlr_color_repr_encoding_from_wp(coefficients);
	if (wlr_encoding == WLR_COLOR_REPR_ENCODING_UNKNOWN) {
		wl_resource_post_error(resource,
			WP_COLOR_REPRESENTATION_V1_ERROR_UNSUPPORTED_COEFFICIENTS,
			"Unsupported coefficients");
		return;
	}

	enum wlr_color_repr_range wlr_range =
		wlr_color_repr_range_from_wp(range);
	if (wlr_range == WLR_COLOR_REPR_RANGE_UNKNOWN) {
		wl_resource_post_error(resource,
			WP_COLOR_REPRESENTATION_V1_ERROR_UNSUPPORTED_COEFFICIENTS,
			"Unsupported range");
		return;
...
static void color_repr_handle_set_chroma_location(struct wl_client *client,
	...
	enum wlr_color_repr_chroma_loc wlr_loc =
		wlr_color_repr_chroma_loc_from_wp(chroma_location);

	if (wlr_loc == WLR_COLOR_REPR_CHROMA_LOC_UNKNOWN) {
		wl_resource_post_error(resource,
			WP_COLOR_REPRESENTATION_V1_ERROR_UNSUPPORTED_CHROMA_LOCATION,
			"Unsupported chroma location");
		return;
	}
...
static void surface_synced_init_state(void *_state) {
	...
	state->coefficients = WLR_COLOR_REPR_ENCODING_UNSET;
	state->range = WLR_COLOR_REPR_RANGE_UNSET;
	state->chroma_location = WLR_COLOR_REPR_CHROMA_LOC_UNSET;
}
...
static void color_repr_manager_handle_create(struct wl_client *client,
	...
	if (color_repr_from_surface(surface) != NULL) {
		wl_resource_post_error(manager_resource,
			WP_COLOR_REPRESENTATION_MANAGER_V1_ERROR_ALREADY_CONSTRUCTED,
			"wp_color_representation_v1 for this surface already exists");
		return;
	}
	...
	color_repr->resource = wl_resource_create(client,
		&wp_color_representation_v1_interface, version, color_repr_id);
...
static const struct wp_color_representation_manager_v1_interface color_repr_manager_impl = {
	...
	.create = color_repr_manager_handle_create,
};
...
static void send_supported(struct wlr_color_representation_manager_v1 *manager,
	...
	wp_color_representation_manager_v1_send_supported_alpha_mode(resource,
		WP_COLOR_REPRESENTATION_V1_ALPHA_MODE_PREMULTIPLIED_ELECTRICAL);

	wp_color_representation_manager_v1_send_supported_chroma_location(resource,
		WP_COLOR_REPRESENTATION_V1_CHROMA_LOCATION_TYPE_0);
	wp_color_representation_manager_v1_send_supported_chroma_location(resource,
		WP_COLOR_REPRESENTATION_V1_CHROMA_LOCATION_TYPE_1);
	wp_color_representation_manager_v1_send_supported_chroma_location(resource,
		WP_COLOR_REPRESENTATION_V1_CHROMA_LOCATION_TYPE_2);
	wp_color_representation_manager_v1_send_supported_chroma_location(resource,
		WP_COLOR_REPRESENTATION_V1_CHROMA_LOCATION_TYPE_3);

	wp_color_representation_manager_v1_send_supported_coefficients_and_ranges(
		resource, WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT601,
		WP_COLOR_REPRESENTATION_V1_RANGE_FULL);
	wp_color_representation_manager_v1_send_supported_coefficients_and_ranges(
		resource, WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT601,
		WP_COLOR_REPRESENTATION_V1_RANGE_LIMITED);
	wp_color_representation_manager_v1_send_supported_coefficients_and_ranges(
		resource, WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT709,
		WP_COLOR_REPRESENTATION_V1_RANGE_FULL);
	wp_color_representation_manager_v1_send_supported_coefficients_and_ranges(
		resource, WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT709,
		WP_COLOR_REPRESENTATION_V1_RANGE_LIMITED);
	wp_color_representation_manager_v1_send_supported_coefficients_and_ranges(
		resource, WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT2020,
		WP_COLOR_REPRESENTATION_V1_RANGE_FULL);
	wp_color_representation_manager_v1_send_supported_coefficients_and_ranges(
		resource, WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT2020,
		WP_COLOR_REPRESENTATION_V1_RANGE_LIMITED);
...
enum wlr_color_repr_encoding wlr_color_repr_encoding_from_wp(int wp_val) {
	switch(wp_val) {
		case WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT601:
			return WLR_COLOR_REPR_ENCODING_BT601;
		case WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT709:
			return WLR_COLOR_REPR_ENCODING_BT709;
		case WP_COLOR_REPRESENTATION_V1_COEFFICIENTS_BT2020:
			return WLR_COLOR_REPR_ENCODING_BT2020;
		default:
			return WLR_COLOR_REPR_ENCODING_UNKNOWN;
	}
}

enum wlr_color_repr_range wlr_color_repr_range_from_wp(int wp_val) {
	switch(wp_val) {
		case WP_COLOR_REPRESENTATION_V1_RANGE_LIMITED:
			return  WLR_COLOR_REPR_RANGE_LIMITED;
		case WP_COLOR_REPRESENTATION_V1_RANGE_FULL:
			return WLR_COLOR_REPR_RANGE_FULL;
		default:
			return WLR_COLOR_REPR_RANGE_UNKNOWN;
	}
}
...
enum wlr_color_repr_chroma_loc wlr_color_repr_chroma_loc_from_wp(int wp_val) {
	switch(wp_val) {
		case WP_COLOR_REPRESENTATION_V1_CHROMA_LOCATION_TYPE_0:
			return WLR_COLOR_REPR_CHROMA_LOC_TYPE0;
		case WP_COLOR_REPRESENTATION_V1_CHROMA_LOCATION_TYPE_1:
			return WLR_COLOR_REPR_CHROMA_LOC_TYPE1;
		case WP_COLOR_REPRESENTATION_V1_CHROMA_LOCATION_TYPE_2:
			return WLR_COLOR_REPR_CHROMA_LOC_TYPE2;
		case WP_COLOR_REPRESENTATION_V1_CHROMA_LOCATION_TYPE_3:
			return WLR_COLOR_REPR_CHROMA_LOC_TYPE3;
		default:
			return WLR_COLOR_REPR_CHROMA_LOC_UNKNOWN;
	}
}
