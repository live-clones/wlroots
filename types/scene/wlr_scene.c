...
static void scene_node_active_outputs(struct wlr_scene_node *node,
		struct wl_list *outputs, struct wlr_scene_output *ignore,
		pixman_region64f_t *visible, uint64_t *active_outputs, size_t *count,
...
static void update_node_update_outputs(struct wlr_scene_node *node,
	...
	scene_node_active_outputs(node, outputs, ignore,
		&node->visible, &active_outputs, &count, &primary_output);
...
static bool construct_render_list_iterator(struct wlr_scene_node *node,
	...
	if (data->scene_output != NULL) {
		...
		uint64_t active_outputs; size_t count;
		struct wlr_scene_output *primary_output = NULL;
		scene_node_active_outputs(scaling_group,
			&data->scene_output->scene->outputs, NULL, &visible,
