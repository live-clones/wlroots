...
static void visible_region_active_outputs(struct wl_list *outputs, struct wlr_scene_output *ignore, pixman_region64f_t *visible,
	...
	if (!pixman_region64f_empty(visible)) {
		...
		pixman_region64f_t visible_region;
		pixman_region64f_init(&visible_region);
		wl_list_for_each(scene_output, outputs, link) {
			int width, height;
			wlr_output_effective_resolution(scene_output->output, &width, &height);
			pixman_region64f_union_rectf(&visible_region, &visible_region,
				scene_output->x, scene_output->y, width, height);
		}
		pixman_region64f_intersect(&visible_region, &visible_region, visible);
		double visible_area = region_area(&visible_region);
		pixman_region64f_fini(&visible_region);
