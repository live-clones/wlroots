...
static bool scene_output_combine_color_transforms(
	...
	if (img_desc != NULL) {
		...
		combined = wlr_color_transform_init_pipeline(transforms, 2);
		wlr_color_transform_unref(transforms[0]);
		wlr_color_transform_unref(transforms[1]);
		if (combined == NULL || transforms[0] == NULL || transforms[1] == NULL) {
			wlr_color_transform_unref(combined);
			return false;
		}
	}
	if (supplied != NULL) {
		assert(combined == NULL);
		combined = wlr_color_transform_ref(supplied);
	}
	if (combined == NULL) {
		combined = wlr_color_transform_init_linear_to_inverse_eotf(
