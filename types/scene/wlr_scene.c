...
static void set_scene_surface(struct wlr_scene_buffer *scene_buffer, int x, int y, void *data) {
	...
	if (scene_surface->surface == timer->surface) {
		...
		wlr_commit_timer_v1_set_output(timer, scene_surface->buffer->primary_output->output);
	}
}
...
static void scene_handle_commit_timing_manager_v1_new_timer(struct wl_listener *listener,
	...
	if (!timer->output.output) {
		...
		wlr_scene_node_for_each_buffer(&scene->tree.node, set_scene_surface, timer);
	}
}
