...
static void surface_reconfigure(struct wlr_scene_surface *scene_surface) {
	...
	if (surface->buffer) {
		...
		struct wlr_scene_buffer_set_buffer_options options = {
			...
			.new_frame = surface->current.committed & WLR_SURFACE_STATE_BUFFER,
		};
		if (syncobj_surface_state != NULL) {
			...
			options.release_timeline = syncobj_surface_state->release_timeline;
			options.release_point = syncobj_surface_state->release_point;
		}
