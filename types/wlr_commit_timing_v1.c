...
static void surface_synced_move_state(void *_dst, void *_src) {
	struct wlr_commit_timer_v1_state *dst = _dst, *src = _src;
	*dst = *src;
	src->refresh = 0;
	src->base_present_nsec = 0;
}

static const struct wlr_surface_synced_impl surface_synced_impl = {
	.state_size = sizeof(struct wlr_commit_timer_v1_state),
	.move_state = surface_synced_move_state,
};

static void commit_remove(struct wlr_commit_timer_v1_commit *commit) {
...
static void timer_handle_output_present(struct wl_listener *listener, void *data) {
	...
	if (timer->state.pending.refresh != event->output->refresh) {
	...
	if (event->presented && !timer->state.pending.base_present_nsec) {
		timer->state.pending.base_present_nsec = timespec_to_nsec(&event->when);
	}
}
...
static uint64_t timer_get_target_nsec(const struct wlr_commit_timer_v1 *timer) {
	...
	if (!output || !timer->state.current.base_present_nsec || !target_nsec ||
	...
	uint64_t cycle_phase_nsec = timer->state.current.base_present_nsec % refresh_nsec;
	...
	if (timer->state.current.refresh) {
		...
		target_nsec -= mhz_to_nsec(timer->state.current.refresh);
	}
...
static void timer_handle_resource_destroy(struct wl_resource *resource) {
	...
	wlr_surface_synced_finish(&timer->synced);
...
static void timing_manager_handle_get_timer(struct wl_client *wl_client,
	...
	wlr_surface_synced_init(&timer->synced, surface, &surface_synced_impl, &timer->state.pending, &timer->state.current);
...
void wlr_commit_timer_v1_set_output(struct wlr_commit_timer_v1 *timer,
	...
	if (output) {
		...
		timer->state.pending.refresh = output->refresh;
