...
static uint64_t timer_get_target_present_nsec(const struct wlr_commit_timer_v1 *timer,
		uint64_t timestamp_nsec) {
	struct wlr_output *output = timer->output;

	// if no output, or not stable yet, we use the requested timestamp as is
	if (!output || !output->refresh || !timer->state.base_present_nsec) {
...
static void timer_handle_client_commit(struct wl_listener *listener, void *data) {
	...
	if (!commit) {
		...
		timer->state.timestamp_nsec = 0;
		goto out;
	}
	commit->timer = timer;
	commit->timestamp_nsec = timer->state.timestamp_nsec;
	timer->state.timestamp_nsec = 0;

	uint64_t target_nsec = timer_get_target_present_nsec(timer, commit->timestamp_nsec);
	if (timer->state.refresh) {
		...
		target_nsec -= mhz_to_nsec(timer->state.refresh); // subtract 1 refresh cycle
		target_nsec -= 1000000; // give a 1msec slop to compensate for possible mismatch
	}

	int64_t delay_target_msec = target_nsec/1000000; // time of target presentation in msec
	...
	wl_event_source_timer_update(commit->unlock_source, delay_target_msec);
...
static void timer_handle_set_timestamp(struct wl_client *client,
	...
	timer->state.timestamp_nsec = timestamp_nsec;
}
