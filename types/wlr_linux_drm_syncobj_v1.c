...
static void surface_synced_commit(struct wlr_surface_synced *synced) {
	...
	// Add at least one default point :
	// While the compositor does not have to protect the client from its own
	// writes, it must not cause a skip when dropping a later commit if an older
	// commit from the same timeline is still in use.
	// The correct value to pass would be to repeat the last point of actual use
	// sent on this timeline. But these points are harder to track, and the client
	// can reasonably be expected to synchronize its render job with this correct
	// release point.
	struct wl_display *display = wl_client_get_display(surface->resource->client);
	wlr_drm_syncobj_merger_add(surface->current.release_merger,
		surface->current.acquire_timeline, surface->current.acquire_point,
