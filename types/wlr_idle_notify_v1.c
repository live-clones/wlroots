...
static void notifier_handle_get_idle_notification(struct wl_client *client,
		...
		struct wl_resource *seat_resource, bool heed_inhibitors) {
	struct wlr_idle_notifier_v1 *notifier =
	...
	notification->heed_inhibitors = heed_inhibitors;
...
static void notifier_handle_get_idle_notification_ignore_inhibitors(
		struct wl_client *client,
		struct wl_resource *notifier_resource, uint32_t id,
		uint32_t timeout, struct wl_resource *seat_resource) {
	notifier_handle_get_idle_notification(client, notifier_resource, id, 
		timeout, seat_resource, false);
}

static void notifier_handle_get_idle_notification_heed_inhibitors(
		struct wl_client *client,
		struct wl_resource *notifier_resource, uint32_t id,
		uint32_t timeout, struct wl_resource *seat_resource) {
	notifier_handle_get_idle_notification(client, notifier_resource, id,
...
static const struct ext_idle_notifier_v1_interface notifier_impl = {
	...
	.get_idle_notification = 
		notifier_handle_get_idle_notification_heed_inhibitors,
	.get_input_idle_notification = 
		notifier_handle_get_idle_notification_ignore_inhibitors,
};
...
void wlr_idle_notifier_v1_set_inhibited(struct wlr_idle_notifier_v1 *notifier,
	...
	wl_list_for_each(notification, &notifier->notifications, link) {
		if (notification->heed_inhibitors) {
			notification_reset_timer(notification);
		}
	}
}
