...
static bool output_group_commit(struct wlr_output *parent, const struct wlr_output_state *state) {
	...
	if (in_tiled_mode && (state->committed & WLR_OUTPUT_STATE_BUFFER)) {
		const struct wlr_fbox *s = &state->buffer_src_box;
		const struct wlr_box *d = &state->buffer_dst_box;
		if (s->x != 0.0 || s->y != 0.0 ||
				(s->width != 0.0 && s->width != state->buffer->width) ||
				(s->height != 0.0 && s->height != state->buffer->height)) {
			wlr_log(WLR_DEBUG, "src_box not yet implemented in output group");
			return false;
		}
		if (d->x != 0 || d->y != 0 ||
				(d->width != 0 && d->width != state->buffer->width) ||
				(d->height != 0 && d->height != state->buffer->height)) {
			wlr_log(WLR_DEBUG, "dst_box not yet implemented in output group");
			return false;
		}
	}
	...
	wl_list_for_each(child, &group->children, link) {
		...
		if (in_tiled_mode) {
			...
			pending->buffer_dst_box.width = (int)child->src_box.width;
			pending->buffer_dst_box.height = (int)child->src_box.height;
...
static void output_group_state_change(void *data) {
	...
	wlr_log(WLR_INFO, "MOO ABOUT TO CLONE");

	if (!clone_output(group, primary->output, &primary->tile_info))
		return;

	wlr_log(WLR_INFO, "MOO AFTER CLONE");
