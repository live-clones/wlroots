...
static struct wlr_ext_foreign_toplevel_image_capture_source_manager_v1 *foreign_toplevel_manager_from_resource(
		struct wl_resource *resource) {
	assert(wl_resource_instance_of(resource,
		&ext_foreign_toplevel_image_capture_source_manager_v1_interface, &foreign_toplevel_manager_impl));
...
wlr_ext_foreign_toplevel_image_capture_source_manager_v1_create(struct wl_display *display, uint32_t version) {
	...
	struct wlr_ext_foreign_toplevel_image_capture_source_manager_v1 *manager = calloc(1, sizeof(*manager));
	if (manager == NULL) {
		return NULL;
	}

	manager->global = wl_global_create(display,
		&ext_foreign_toplevel_image_capture_source_manager_v1_interface, version, manager, foreign_toplevel_manager_bind);
