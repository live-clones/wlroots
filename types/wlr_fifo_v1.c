...
static void fifo_handle_client_commit(struct wl_listener *listener, void *data) {
	...
	if (!fifo->surface || !should_queue_commit(fifo)) {
		return;
	}
...
static void fifo_destroy(struct wlr_fifo_v1 *fifo) {
	...
	struct wl_resource *fifo_resource = fifo->resource;
	fifo_handle_resource_destroy(fifo->resource);
	wl_resource_set_user_data(fifo_resource, NULL);
}
