...
static int handle_timer(void *data) {
	...
	int64_t now_nsec = get_current_time_nsec();
	// if 1 second has passed since we had any of the surface commit presented, advance the queue
	// through the timer.
	if (fifo->barrier_set && (now_nsec - (int64_t)fifo->last_output_present_nsec > 1000000000)) {
...
static void fifo_reset(struct wlr_fifo_v1 *fifo) {
	...
	if (fifo->surface_output) {
		fifo->surface_output_commit_present.notify = NULL;
		wl_list_remove(&fifo->surface_output_commit_present.link);
...
static void fifo_handle_surface_output_commit_present(struct wl_listener *listener, void *data) {
	struct wlr_fifo_v1 *fifo =
		wl_container_of(listener, fifo, surface_output_commit_present);
	struct wlr_surface_output_event_present *event = data;

	if (!fifo->surface->buffer) {
		return;
	}

	if (fifo->barrier_set && event->commit->committed_on_output) {
		fifo_signal_barrier(fifo);
	}

	if (event->commit->presented_on_output) {
...
static void fifo_handle_client_commit(struct wl_listener *listener, void *data) {
	...
	if (!fifo->surface || !fifo->surface_output || !is_surface_buffer_valid(fifo->surface)) {
		return;
	}
...
void wlr_fifo_v1_set_output(struct wlr_fifo_v1 *fifo, struct wlr_output *output) {
	...
	if (!output || (fifo->surface_output && output == fifo->surface_output->output)) {
		return;
	}
	...
	fifo->surface_output = surface_output;
	fifo->output_destroy.notify = fifo_handle_output_destroy;
	fifo->surface_output_commit_present.notify = fifo_handle_surface_output_commit_present;
	wl_signal_add(&fifo->surface_output->events.present, &fifo->surface_output_commit_present);
	wl_signal_add(&fifo->surface_output->output->events.destroy, &fifo->output_destroy);
}
