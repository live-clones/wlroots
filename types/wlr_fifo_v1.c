...
static int handle_timer(void *data) {
	...
	// set the timer to twice the frequency of the output, or to 40hz if we have no output
	int64_t refresh_msec = fifo->output ? 1000000/fifo->output->refresh*2 : 25;
	int64_t now = get_current_time_msec();
	if (now - fifo->last_output_commit_msec > refresh_msec && fifo->barrier_set) {
		fifo_signal_barrier(fifo);
	}
	wl_event_source_timer_update(fifo->unlock_timer, refresh_msec);
	return 0;
}
...
static bool fifo_do_queue_commit(struct wlr_fifo_v1 *fifo) {
	...
	return (!wl_list_empty(&fifo->commits) ||
			(fifo->pending.wait_barrier &&
			 (fifo->barrier_set ||
			  (!fifo->barrier_set && pending_set_barrier))));
...
static void fifo_handle_resource_destroy(struct wl_resource *resource) {
	...
	if (fifo->unlock_timer) {
		wl_event_source_remove(fifo->unlock_timer);
	}
	free(fifo);
}
...
static void fifo_manager_handle_get_fifo(struct wl_client *wl_client, struct wl_resource *resource,
	...
	// We constantly run a timer to advance the queue in the following cases:
	//  - initially we might not have an output to which we can latch commits onto to advance the
	//  queue.
	//  - we have an output, but the surface commits are not latched onto it because the surface is
	//  occluded.
	fifo->unlock_timer =
