...
int main(int argc, char *argv[]) {
	...
	struct wlr_backend *backend = wlr_headless_backend_create(ev);
	assert(backend);
	...
	for (int i = 1; i <= iters; ++i) {
		...
		int duration = -1;
		while ((duration = wlr_render_timer_get_duration_ns(timer)) == -1);

		wlr_log(WLR_INFO, "wlr_render_pass iteration %d/%d (%d rects): %d ns", i, iters, rects,
			duration);
