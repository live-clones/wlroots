...
void parse_edid(struct wlr_drm_connector *conn, size_t len, const uint8_t *data) {
	...
	output->supported_transfer_functions |= WLR_OUTPUT_TRANSFER_FUNCTION_BT1886;
	if (conn->props.hdr_output_metadata != 0 && hdr_static_metadata->type1 && hdr_static_metadata->pq) {
		output->supported_transfer_functions |= WLR_OUTPUT_TRANSFER_FUNCTION_ST2084_PQ;
	}
	...
	output->supported_primaries |= WLR_OUTPUT_PRIMARIES_SRGB;
	if (conn->props.colorspace != 0 && has_bt2020) {
		output->supported_primaries |= WLR_OUTPUT_PRIMARIES_BT2020;
	}
